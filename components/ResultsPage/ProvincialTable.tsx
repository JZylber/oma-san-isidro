    ...
    const make_element = (result : ProvincialResult,index : number) => {
        ...
        return(
            <tr key={index}>
                ...
                <td>{school}</td>
                <td>{price}</td>
                {passed?<td className={styles.center_align}>{result.aprobado?"Sí":"No"}</td>:null}
            </tr>)
    }
    ...
    return(
        <div className={styles.container}>
            <form className={styles.form}>
                ...
                {passed?<SelectResultCategory category="Aprobado" value={resultFilter.aprobado} setValue={(option?: boolean) => updateFilter({aprobado: option})} options={options.aprobado} clear/>:null}
            </form>
