...
const ResultFinderForm = ({ availableResults, data, setData }: FormProps) => {
  ...
  const resultYears = availableResults
    .map((yearTests) => yearTests.ano)
    .sort()
    .reverse();
  useEffect(() => {
    if (!checkRoute && query) {
      ...
      if (año && instancia) {
        ...
        if (
          resultYears.includes(year) &&
          availableResults
            .find((result) => result.ano === year)
            ?.pruebas.find((instance) => instance.nombre === instancia)
        ) {
          setData({
            año: year,
            instancia: instance.toUpperCase() as INSTANCIA,
          });
        }
      }
      setCheckRoute(true);
  ...
  let possibleInstances = data.año
    ? (availableResults.find((result) => result.ano === data.año) as yearTests)
        .pruebas
    : [];
  ...
  return (
    <form className={styles.form}>
      ...
      <SelectResultCategory
        category="Instancia"
        value={capitalizeIfNotUndefined(data.instancia)}
        setValue={setInstance}
        options={instances.map(capitalize)}
