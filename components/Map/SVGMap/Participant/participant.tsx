...
const Participant = ({school,level, size = 40, selected = false}: ParticipantProps) => {
    ...
    return (
        ...
        <Tooltip id="participant" place="right">
            <div className="flex font-montserrat divide-x-2" style={{fontSize:`${size/3}px`}}>
                <span className='pr-2'>{level}</span>
                <span className='pl-2'>{school.name}</span>
            </div>
        </Tooltip>
        </>
    )
