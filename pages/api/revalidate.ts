...
export default async function handler(req : NextApiRequest, res:NextApiResponse) {
    ...
    if(req.query.type === "results"){
      ...
      try {
        await res.revalidate('/calendario')
        await res.revalidate('/otros/geometria')
        await res.revalidate('/')
        return res.json({ revalidated: true })
      } catch (err) {
        return res.status(500).send('Error revalidating')
    ...
    } else if(req.query.type === "inscriptions"){
      try {
        await res.revalidate('/oma/inscripciones')
        await res.revalidate('/nandu/inscripciones')
        return res.json({ revalidated: true })
      } catch (err) {
        return res.status(500).send('Error revalidating')
      }
