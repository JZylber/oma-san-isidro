...
export class Problems implements Filterable<Problems> {
    ...
    isFilteredBy(element: Problems): boolean {
        return (
            ...
            this.problems.every((problem, index) => problem.isFilteredBy(element.problems[index]))
        );
    }
    toString(): string {
        return `${this.problems.map((problem) => problem.toString()).join(', ')},${this.total}`;
    }
}

export type Filterables = string | number | boolean | Filterable<any>;

export type FilterableObject = Record<string,Filterables>;
